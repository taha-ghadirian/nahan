...
async function handleUpdateApi(request, env, ctx) {
    try {
        ...
        const repo = (sysConfig.githubRepo || "taha-ghadirian/nahan").replace(/https?:\/\/github\.com\//, '').trim();
...
function getAllProfiles(targetId = null, targetSub = null) {
    ...
    if (targetSub) {
        list = list.filter(p => p.name.toLowerCase() === targetSub.toLowerCase() && p.id == targetId);
    }
    return list;
}
                  ...
                  <div class="max-w-4xl mx-auto space-y-6 fade-in">
                      ...
                      <div id="update-alert-banner" class="hidden bg-gradient-to-r from-amber-500/10 to-primary/10 border-2 border-amber-300 dark:border-amber-950/20 rounded-3xl p-6 shadow-md flex-col items-center justify-between gap-4 fade-in">
                          ...
                          <div class="w-full mt-2 text-center">
                              <a id="update-github-link" href="https://github.com/taha-ghadirian/nahan" target="_blank" class="text-xs text-slate-400 hover:text-amber-500 transition-colors underline" data-i18n="view_github">View on GitHub</a>
                          </div>
                      </div>
...
function buildPortCheckboxes(wrapId, selectedPorts) {
          ...
          async function checkUpdate() {
              try {
                  ...
                  if (data.success && data.updateAvailable) {
                      ...
                      showUpdateBanner((document.getElementById('cfg-github-repo')?.value || window.nahanConfig?.githubRepo || 'taha-ghadirian/nahan').replace('https://github.com/', '').replace('http://github.com/', '').trim(), data.latest);
                  }
          ...
          async function doUpdate() {
              ...
              let latestCode = null;
              try {
                  const repo = (document.getElementById('cfg-github-repo')?.value || window.nahanConfig?.githubRepo || 'taha-ghadirian/nahan').replace('https://github.com/', '').replace('http://github.com/', '').trim();
